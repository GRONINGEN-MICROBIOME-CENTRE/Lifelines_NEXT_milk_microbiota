################################################################################################################
### INVESTIGATE SHARING OF BACTERIA BETWEEN MILK AND INFANT FAECES - SCRIPT 3 - M3  (FIGURE 6A - PART 2) #######
################################################################################################################
...
# 3. GENERATE PLOTS (FIGURE 6A - PART 2)
# 4. SAVE RESULTS
...
# Read results files
final_PSE_data_M3 <- read.delim("M3/240219_PSE_table_maternal_milk_infant_faeces_M3.txt", header = T, check.names = F) 
cont_tables_M3 <- readRDS("M3/240219_maternal_milk_infant_faeces_contingency_tables_M3.rds")
...
pdf('M3/240219_plots_PSE_relatedness_mother_milk_infant_faeces_M3.pdf', width=30, height=5)
...
## detailed table
write.table(final_PSE_data_summary_long_M3, file ="M3/240219_mother_milk_infant_faeces_final_PSE_results_M3.txt", row.names=F, col.names=T, sep="\t", quote=F)
...
write.table(final_PSE_data_summary_M3_v3, file ="M3/240219_mother_milk_infant_faeces_final_PSE_results_short_M3.txt", row.names=F, col.names=T, sep="\t", quote=F)
